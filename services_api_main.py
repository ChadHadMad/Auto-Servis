...
def admin_set_role(user_id: UUID, payload: RoleUpdateIn, db: Session = Depends(get_db), admin=Depends(require_role("admin"))):
...
def admin_delete_user(user_id: UUID, db: Session = Depends(get_db), admin=Depends(require_role("admin"))):
...
def admin_create_order(payload: AdminOrderCreate, db: Session = Depends(get_db), admin=Depends(require_role("admin"))):
    ...
    created = crud.create_order(db, customer.id, customer_name, vehicle_text, payload.service_date, notes=payload.notes, vehicle_id=vehicle_id)
    delete(CACHE_KEY_ALL_ORDERS)
    return created


@app.get("/admin/vehicles", response_model=list[VehicleOut])
def admin_list_customer_vehicles(customer_email: str, db: Session = Depends(get_db), admin=Depends(require_role("admin"))):
...
def admin_add_vehicle_for_customer(payload: AdminVehicleCreate, db: Session = Depends(get_db), admin=Depends(require_role("admin"))):
    customer = crud.get_user_by_email(db, payload.customer_email)
    if not customer:
        raise HTTPException(status_code=404, detail="Customer not found")
    if not payload.plate or not payload.plate.strip():
        raise HTTPException(status_code=400, detail="Plate is required")
    return crud.create_vehicle(db, customer.id, payload.make.strip(), payload.model.strip(), payload.plate.strip(), payload.year)
...
def create_order(payload: OrderCreate, db: Session = Depends(get_db), user=Depends(require_role("customer"))):
    ...
    vehicle_text = f"{v.make} {v.model}" + (f" ({v.plate})" if v.plate else "") + (f" - {v.year}" if v.year else "")
    created = crud.create_order(db, user.id, customer_name, vehicle_text, payload.service_date, notes=payload.notes, vehicle_id=v.id)
    delete(CACHE_KEY_ALL_ORDERS)
    return created


@app.get("/orders", response_model=list[OrderOut])
def list_orders(status: str | None = Query(default=None), service_date: date_type | None = Query(default=None), db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def update_status(order_id: UUID, status: str, db: Session = Depends(get_db), user=Depends(require_role("mechanic", "admin"))):
    if status not in ALLOWED_STATUSES:
        raise HTTPException(status_code=400, detail="Invalid status")
    updated = crud.update_order_status(db, order_id, status)
    if not updated:
        raise HTTPException(status_code=404, detail="Order not found")
    publish_status_event({"event": "order_status_changed", "order_id": str(updated.id), "new_status": updated.status, "service_date": str(updated.service_date), "customer_name": updated.customer_name, "vehicle": updated.vehicle})
    delete(CACHE_KEY_ALL_ORDERS)
    return updated


@app.delete("/orders/{order_id}", response_model=OrderOut)
def cancel_order(order_id: UUID, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    if not cancelled:
        raise HTTPException(status_code=404, detail="Order not found")
    publish_status_event({"event": "order_status_changed", "order_id": str(cancelled.id), "new_status": cancelled.status, "service_date": str(cancelled.service_date), "customer_name": cancelled.customer_name, "vehicle": cancelled.vehicle})
    delete(CACHE_KEY_ALL_ORDERS)
    return cancelled
